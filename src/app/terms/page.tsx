...
export default function TermsPage() {
  return (
    <PageShell
      ...
      subtitle="These terms include exchange and returns conditions as shared."
      contentClassName="pt-10"
    >
      <section className="rounded-3xl border border-black/10 bg-white/70 p-6 shadow-sm sm:p-8">
        <ol className="list-decimal space-y-3 pl-5 text-sm leading-relaxed text-neutral-700 sm:text-base">
          {TERMS_POINTS.map((p) => (
            <li key={p}>{p}</li>
          ))}
        </ol>
