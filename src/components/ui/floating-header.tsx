    ...
    <header className="w-full">
      ...
      >
          ...
          {PRIMARY_NAV.filter((x) => x.name !== "Cart").map((item) =>
            item.items && item.items.length > 0 ? (
              ...
              >
                <a
                  href={item.href}
                  className="inline-flex items-center gap-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-zinc-700 hover:text-zinc-900 transition-colors"
                  aria-haspopup="menu"
              ...
              <a
                key={item.name}
                href={item.href}
                className="text-[10px] font-semibold uppercase tracking-[0.14em] text-zinc-700 hover:text-zinc-900 transition-colors"
              >
                {item.name}
              </a>
            ),
        ...
        >
          ...
          <motion.button
            type="submit"
            className="rounded-full border border-zinc-300 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.12em] text-zinc-800 hover:bg-zinc-100"
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          <motion.a
            ...
            className="relative rounded-full px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-zinc-700 hover:text-zinc-900 sm:px-3"
            whileHover={reduceMotion ? undefined : { y: -1 }}
          >
            Cart
            {itemCount > 0 ? (
              <span className="ml-2 inline-flex min-w-5 items-center justify-center rounded-full bg-zinc-900 px-1.5 py-0.5 text-[10px] font-bold text-white">
                {itemCount}
              </span>
            ) : null}
          </motion.a>
          {!isLoggedIn ? (
            <motion.a
              href="/sign-in"
              className="hidden rounded-full border border-zinc-300 bg-transparent px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-zinc-900 hover:bg-zinc-100 md:inline-flex"
        ...
        {menuOpen ? (
          ...
          <nav className="flex flex-col gap-2">
            ...
            >
              ...
              <button
                type="submit"
                className="rounded-full border border-zinc-300 px-3 py-2 text-[10px] font-semibold uppercase tracking-[0.12em] text-zinc-800"
              >
                Go
              </button>
            </form>
              ...
              {PRIMARY_NAV.filter((x) => x.name !== "Cart").map((item) =>
                item.items && item.items.length > 0 ? (
                  <div key={item.name} className="space-y-1">
                    <a
                      href={item.href}
                      className="block rounded-xl px-3 py-2 text-[10px] font-semibold uppercase tracking-[0.16em] text-zinc-800 hover:bg-zinc-50"
                      onClick={() => setMenuOpen(false)}
                    >
                      {item.name}
                    </a>
